...
from crawlers import VapeMonsterCrawler, VapingLabCrawler, Juice24Crawler, Juice99Crawler, JuiceboxCrawler, JuiceshopCrawler, SkyVapeCrawler, KimiVapeCrawler, JuicegramCrawler, \
    Vape49Crawler, LoungeVapeCrawler, Juice79Crawler, BreathingKoreaCrawler, Vape9Crawler
# 로깅 모듈 가져오기
...
def run_crawler(crawler_class, keywords, headless, categories=None, env_file='.env'):
    ...
    crawler = None
    try:
        # VapeMonsterCrawler, VapingLabCrawler, Juice24Crawler, Juice99Crawler, JuiceboxCrawler, JuiceshopCrawler, SkyVapeCrawler, KimiVapeCrawler, JuicegramCrawler, Vape49Crawler, LoungeVapeCrawler, Juice79Crawler, BreathingKoreaCrawler, Vape9Crawler인 경우 카테고리 처리
        if (crawler_class in [VapeMonsterCrawler, VapingLabCrawler, Juice24Crawler, Juice99Crawler, JuiceboxCrawler, JuiceshopCrawler, SkyVapeCrawler, KimiVapeCrawler, JuicegramCrawler, Vape49Crawler, LoungeVapeCrawler, Juice79Crawler, BreathingKoreaCrawler, Vape9Crawler]) and categories and len(
                categories) > 0:
            # 첫 번째 카테고리로 인스턴스 생성
...
def main():
    ...
    # 명령줄 인수 파싱
    parser = argparse.ArgumentParser(description='VapeCrawler - A modular web crawler for vape products')
    parser.add_argument('--sites', nargs='+', choices=['vapemonster', 'vapinglab', 'juice24', 'juice99', 'juicebox', 'juiceshop', 'skyvape', 'kimivape', 'juicegram', 'vape49', 'loungevape', 'juice79', 'breathingkorea', 'vape9', 'all'], default=['all'],
    ...
    parser.add_argument('--categories', nargs='+', choices=['입호흡', '폐호흡'],
                        help='Categories to crawl (VapeMonster: 입호흡, 폐호흡 / VapingLab: 입호흡, 폐호흡 / Juice24: 입호흡, 폐호흡 / Juice99: 입호흡, 폐호흡 / Juicebox: 입호흡, 폐호흡 / Juiceshop: 입호흡, 폐호흡 / SkyVape: 입호흡, 폐호흡 / KimiVape: 입호흡, 폐호흡 / Juicegram: 입호흡, 폐호흡 / Vape49: 입호흡, 폐호흡 / LoungeVape: 입호흡, 폐호흡 / Juice79: 입호흡, 폐호흡 / BreathingKorea: 입호흡, 폐호흡 / Vape9: 입호흡, 폐호흡)')
    ...
    if 'all' in args.sites:
        sites_to_crawl = ['vapemonster', 'vapinglab', 'juice24', 'juice99', 'juicebox', 'juiceshop', 'skyvape', 'kimivape', 'juicegram', 'vape49', 'loungevape', 'juice79', 'breathingkorea', 'vape9']
    else:
    ...
    for site in sites_to_crawl:
        if site in crawler_map:
            # 스레드 생성
            if (site == 'vapemonster' or site == 'vapinglab' or site == 'juice24' or site == 'juice99' or site == 'juicebox' or site == 'juiceshop' or site == 'kimivape' or site == 'juicegram' or site == 'vape49' or site == 'loungevape' or site == 'juice79' or site == 'breathingkorea' or site == 'vape9') and args.categories:
