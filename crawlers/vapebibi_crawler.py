...
class VapebibiCrawler(BaseCrawler):
    ...
    def get_products(self):
        if not self.navigate_to(self.category_url):
            self.logger.error("전담액상비비 카테고리 페이지로 이동 실패")
            return []
        time.sleep(2)
        products = []
        current_page = 1
        while True:
            self.logger.info(f"전담액상비비 {self.category} {current_page}페이지 크롤링 중")
            time.sleep(2)
            ...
            for element in product_elements:
                try:
                    ...
                    # 가격 (세 번째 span에 판매가가 노출되는 케이스를 우선 처리)
                    try:
    ...
    def crawl(self, keywords=None, categories=None):
        self.logger.info("전담액상비비 크롤링 시작")
        results = {}
        ...
        for category in categories:
            if category != self.category:
                ...
                crawler = VapebibiCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
