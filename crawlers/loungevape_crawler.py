...
class LoungeVapeCrawler(BaseCrawler):
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("LoungeVape 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"LoungeVape {self.category} 카테고리의 {current_page} 페이지 크롤링 중")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        """
        self.logger.info("LoungeVape 크롤링 시작")
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = LoungeVapeCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
