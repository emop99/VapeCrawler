...
class MamavapeCrawler(BaseCrawler):
    ...
    def get_products(self):
        if not self.navigate_to(self.category_url):
            self.logger.error("마마베이프 카테고리 페이지로 이동 실패")
            return []
        time.sleep(2)
        products = []
        current_page = 1
        while True:
            self.logger.info(f"마마베이프 {self.category} {current_page}페이지 크롤링 중")
            time.sleep(2)
            ...
            for element in product_elements:
                try:
                    ...
                    # 이미지
                    try:
                        img_element = element.find_element(By.CSS_SELECTOR, "div.thumbnail a[name^='anchorBoxName_'] img")
            ...
            try:
                ...
                time.sleep(2)
                # 다음 페이지에서 더 이상 제품이 없다면 종료할 수 있도록 한 번 더 확인하도록 루프에 맡김
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        for category in categories:
            if category != self.category:
                ...
                crawler = MamavapeCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
