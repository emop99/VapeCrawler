...
class Vape49Crawler(BaseCrawler):
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("49vape 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"49vape {self.category} 카테고리의 {current_page} 페이지 크롤링 중")
            ...
            for element in product_elements:
                try:
                    ...
                    # 제품 가격 추출
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, ".description ul.spec span:nth-child(2)")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        """
        self.logger.info("49vape 크롤링 시작")
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = Vape49Crawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
