...
class JuicegramCrawler(BaseCrawler):
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("쥬스그램 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"쥬스그램 {self.category} 카테고리의 {current_page} 페이지 크롤링 중")

            # 제품 요소가 로드될 때까지 대기
            time.sleep(2)

            # 모든 제품 요소 찾기
            product_elements = self.find_elements(By.CSS_SELECTOR, ".sp-product-box .sp-product-item")
            ...
            for element in product_elements:
                try:
                    ...
                    # 제품 제목 추출
                    try:
                        title_element = element.find_element(By.CSS_SELECTOR, ".sp-product-name span:nth-child(2)")
                    ...
                    # 제품 가격 추출
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, ".sp-product-description div[rel='판매가']")
                    ...
                    # 제품 URL 추출
                    try:
                        url_element = element.find_element(By.CSS_SELECTOR, ".sp-product-name a")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = JuicegramCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
