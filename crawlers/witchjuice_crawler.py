...
class WitchjuiceCrawler(BaseCrawler):
    ...
    def get_products(self):
        if not self.navigate_to(self.category_url):
            self.logger.error("마녀쥬스 카테고리 페이지로 이동 실패")
            return []
        time.sleep(2)
        products = []
        current_page = 1
        while True:
            self.logger.info(f"마녀쥬스 {self.category} {current_page}페이지 크롤링 중")
            time.sleep(2)
            ...
            for element in product_elements:
                try:
                    ...
                    price_str = "N/A"
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, ".description ul.spec li span:nth-child(2)")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        for category in categories:
            if category != self.category:
                ...
                crawler = WitchjuiceCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
