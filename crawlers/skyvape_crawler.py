...
class SkyVapeCrawler(BaseCrawler):
    ...
    def __init__(self, headless=True, category="입호흡"):
        ...
        """
        super().__init__("skyvape", headless)
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("스카이베이프 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"스카이베이프 {self.category} 카테고리의 {current_page} 페이지 크롤링 중")

            # 제품 요소가 로드될 때까지 대기
            time.sleep(2)

            # 모든 제품 요소 찾기 (.prdList 내부의 제품 요소)
            product_elements = self.find_elements(By.CSS_SELECTOR, ".prdList li")
            ...
            for element in product_elements:
                try:
                    # 제품 제목 추출
                    try:
                        title_element = element.find_element(By.CSS_SELECTOR, ".name a")
                    ...
                    # 제품 가격 추출
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, "ul span")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        """
        self.logger.info("스카이베이프 크롤링 시작")
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = SkyVapeCrawler(headless=self.headless, category=category)
                try:
