...
class JuiceboxCrawler(BaseCrawler):
    ...
    def __init__(self, headless=True, category="입호흡"):
        ...
        """
        super().__init__("juicebox", headless)
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("주스박스 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"주스박스 {self.category} 카테고리의 {current_page} 페이지 크롤링 중")

            # 제품 요소가 로드될 때까지 대기
            time.sleep(2)

            # 모든 제품 요소 찾기 (.prdList__item)
            product_elements = self.find_elements(By.CSS_SELECTOR, ".prdList__item")
            ...
            for element in product_elements:
                try:
                    # 제품 제목 추출 (.description .name a 선택자에 마지막 span)
                    try:
                        title_element = element.find_element(By.CSS_SELECTOR, ".description .name a")
                        # 마지막 span 요소 찾기
                        span_elements = title_element.find_elements(By.CSS_SELECTOR, "span")
                        if span_elements and len(span_elements) > 0:
                            # 마지막 span 요소의 텍스트 가져오기
                            last_span = span_elements[-1]
                            title = last_span.text.strip()
                        else:
                            # span이 없으면 전체 텍스트 사용
                    ...
                    # 제품 가격 추출 (.description .spec_wrap .product_price 선택자에 두번째 span)
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, ".description .spec_wrap .product_price span:nth-child(2)")
                    ...
                    # 제품 URL 추출 (.description .name a 선택자에 attribute href)
                    try:
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = JuiceboxCrawler(headless=self.headless, category=category)
                try:
