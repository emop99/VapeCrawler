...
class TfnmallCrawler(BaseCrawler):
    ...
    카테고리:
      - 입호흡: https://tfnmall.com/product/list.html?cate_no=44
      - 폐호흡: https://tfnmall.com/product/list.html?cate_no=45
    페이징: page 파라미터 사용
    """
    ...
    def get_products(self):
        if not self.navigate_to(self.category_url):
            self.logger.error("티에프몰 카테고리 페이지로 이동 실패")
            return []
        time.sleep(2)
        products = []
        current_page = 1
        while True:
            self.logger.info(f"티에프몰 {self.category} {current_page}페이지 크롤링 중")
            time.sleep(2)
            ...
            for element in product_elements:
                try:
                    ...
                    # 제목
                    try:
                        title_element = element.find_element(By.CSS_SELECTOR, ".description p.name a span:nth-child(2)")
                    ...
                    price_str = "N/A"
                    try:
                        # 'data-price' 속성 값은 '원가^판매가' 형식
                        data_price_attr = element.get_attribute('data-price')
                        if data_price_attr:
                            # '^' 문자를 기준으로 분리하고 2번째 값(가격)을 추출합니다.
                            price_str = data_price_attr.split('^')[1]
                    except Exception as e:
                    ...
                    # 이미지
                    try:
                        img_element = element.find_element(By.CSS_SELECTOR, "div.thumbnail a .add_thumb img")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        for category in categories:
            if category != self.category:
                ...
                crawler = TfnmallCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
