"""
펭펭쥬스(pengjuice) 크롤러 구현.
사이트: https://pengjuice.kr/
...
class PengJuiceCrawler(BaseCrawler):
    """
    Cafe24 ApiProductNormal JSON API를 이용하여 제품을 수집합니다.
    """

    SITE_KO = "펭펭쥬스"
    BASE_HOST = "https://pengjuice.kr"
    API_TEMPLATE = (
            BASE_HOST +
    ...
    def __init__(self, headless=True, category="입호흡", env_file='.env'):
        super().__init__("pengjuice", headless, env_file=env_file)
        ...
        while page <= max_pages:
            ...
            for item in items:
                try:
                    ...
                    title = item.get('product_name_striptag') or item.get('disp_product_name') or ""
            ...
            if data.get('rtn_data', {}).get('end') is True:
                break
