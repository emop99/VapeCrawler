...
class Vape365Crawler(BaseCrawler):
    ...
    def get_products(self):
        if not self.navigate_to(self.category_url):
            self.logger.error("베이프365 카테고리 페이지로 이동 실패")
            return []
        time.sleep(2)
        products = []
        current_page = 1
        while True:
            self.logger.info(f"베이프365 {self.category} {current_page}페이지 크롤링 중")
            time.sleep(2)
    ...
    def crawl(self, keywords=None, categories=None):
        self.logger.info("베이프365 크롤링 시작")
        results = {}
        ...
        for category in categories:
            if category != self.category:
                ...
                crawler = Vape365Crawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
