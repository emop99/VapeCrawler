...
class Juice79Crawler(BaseCrawler):
    ...
    def __init__(self, headless=True, category="입호흡", env_file='.env'):
        ...
        self.category_url = f"{self.base_url}/category/%EC%9E%85%ED%98%B8%ED%9D%A1-%EC%95%A1%EC%83%81/{category_path}/" if category == "입호흡" else f"{self.base_url}/category/%ED%8F%90%ED%98%B8%ED%9D%A1-%EC%95%A1%EC%83%81/{category_path}/"
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("79juice 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"79juice {self.category} 카테고리의 {current_page} 페이지 크롤링 중")

            # 제품 요소가 로드될 때까지 대기
            time.sleep(2)

            # 모든 제품 요소 찾기
            product_elements = self.find_elements(By.CSS_SELECTOR, ".prdList.grid4 [id^='anchorBoxId_']")
            ...
            for element in product_elements:
                try:
                    ...
                    # 제품 가격 추출
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, ".description .prd_price_sale span:nth-child(2)")
                        price_str = price_element.text.strip() if price_element else "N/A"
                    except Exception as e:
                        # 가격 요소가 없을 경우 대체 요소 찾기
                        try:
                            price_element = element.find_element(By.CSS_SELECTOR, ".description .product_price")
                            price_str = price_element.text.strip() if price_element else "N/A"
                        except Exception as e:
                    ...
                    # 제품 URL 추출
                    try:
                        url_element = element.find_element(By.CSS_SELECTOR, ".thumbnail a")
                    ...
                    # 제품 이미지 URL 추출
                    # try:
                    #     img_element = element.find_element(By.CSS_SELECTOR, "img.thumb")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        """
        self.logger.info("79juice 크롤링 시작")
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = Juice79Crawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
