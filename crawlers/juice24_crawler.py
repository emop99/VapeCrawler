...
class Juice24Crawler(BaseCrawler):
    ...
    def __init__(self, headless=True, category="입호흡"):
        ...
        """
        super().__init__("juice24", headless)
    ...
    def get_products(self):
        ...
        if not self.navigate_to(self.category_url):
            self.logger.error("주스24 카테고리 페이지로 이동하지 못했습니다")
            return []
        ...
        while has_products:
            self.logger.info(f"주스24 {self.category} 카테고리의 {current_page} 페이지 크롤링 중")
    ...
    def crawl(self, keywords=None, categories=None):
        ...
        for category in categories:
            ...
            if category != self.category:
                ...
                crawler = Juice24Crawler(headless=self.headless, category=category)
                try:
