...
class VaporwaveCrawler(BaseCrawler):
    ...
    def get_products(self):
        if not self.navigate_to(self.category_url):
            self.logger.error("베이퍼웨이브 카테고리 페이지로 이동 실패")
            return []
        time.sleep(2)
        products = []
        current_page = 1
        while True:
            self.logger.info(f"베이퍼웨이브 {self.category} {current_page}페이지 크롤링 중")
            time.sleep(2)
            ...
            for element in product_elements:
                try:
                    ...
                    # 가격
                    try:
                        price_element = element.find_element(By.CSS_SELECTOR, ".description ul.spec li:nth-child(2)")
                        price_str = price_element.text.strip() if price_element else "N/A"
                    except Exception as e:
                        price_element = element.find_element(By.CSS_SELECTOR, ".description ul.spec li:nth-child(1)")
                        price_str = price_element.text.strip() if price_element else "N/A"
                    ...
                    # 이미지
                    try:
                        img_element = element.find_element(By.CSS_SELECTOR, "div.thumbnail a img.img_small")
            ...
            try:
                ...
                time.sleep(2)
                # 다음 페이지에도 제품이 없다면 종료되도록 루프 상단에서 검사
    ...
    def crawl(self, keywords=None, categories=None):
        self.logger.info("베이핑웨이브 크롤링 시작")
        results = {}
        ...
        for category in categories:
            if category != self.category:
                ...
                crawler = VaporwaveCrawler(headless=self.headless, category=category, env_file=self.env_file)
                try:
